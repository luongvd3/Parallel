	...
	int *penalties)
{
	...
	#pragma omp parallel for schedule(guided,1) shared(genes,alignmentHash,problems) num_threads(8) proc_bind(close)
	for(int i = 0; i < problems.size(); ++i) {
		...
		#pragma omp critical (updateHash) hint(omp_sync_hint_speculative)
		{
